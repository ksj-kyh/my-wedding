...
(function () {
  ...
  function initGreeting() { $('#greetingTitle').textContent = CONFIG.greeting.title; $('#greetingContent').textContent = CONFIG.greeting.content; const g = CONFIG.groom; const b = CONFIG.bride; function familyLine(person) { const parentName = person.mother || person.father || ''; const childOrder = person.childOrder || ''; const isDeceased = person.mother ? person.motherDeceased : person.fatherDeceased; const deceasedClass = isDeceased ? ' deceased' : ''; if (parentName) { return `<span class="${deceasedClass}">${parentName}</span>의 ${childOrder} <span class="child-name">${person.name}</span>`; } return `${childOrder} <span class="child-name">${person.name}</span>`; } const parentsHTML = ` <div class="parent-row"> ${familyLine(g)} </div> <div class="parent-row"> ${familyLine(b)} </div> `; $('#greetingParents').innerHTML = parentsHTML; }
  ...
  function initCalendar() {
    ...
    const monthNames = ['1월', '2월', '3월', '4월', '5월', '6월',
      '7월', '8월', 'September', 'October', 'November', 'December'];
